...
Icmp::Icmp(const std::string &destAddr)
    : m_sockfd(openSocket()), m_pid(::getpid())
{
...
int pack(char *packet, int num, int pid)
{
    ...
    icmp->icmp_cksum = in_cksum((unsigned short *)icmp, packsize); /*校验算法*/
    return packsize;
}
...
size_t Icmp::sendIcmpPacket()
{
    ...
    return sendto(m_sockfd, sendpacket, packsize, 0,
...
bool Icmp::receiveIcmpPacket(struct ResultPacket &resultPacket) const
{
    ...
    int packet_size = recvfrom(m_sockfd, recvpacket, sizeof(recvpacket), 0,
